...
router.get('/:id/doctors', async (req, res) => {
  try {
    ...
    if (!org) return res.status(404).json({ error: 'organization not found' })
      ...
      let data
      try { data = require('jsonwebtoken').verify(token, process.env.JWT_SECRET || 'missing-secret') } catch (err) { return res.status(401).json({ error: 'invalid token' }) }
    ...
    console.log(`Slug : ${org.slug}, Name : ${org.name}`)
    return res.json({ 
      organization: {
        id: String(org._id),
        name: org.name,
...
router.post('/:id/patients/:pid/assign', async (req, res) => {
  try {
    ...
    if (!requesterId) return res.status(401).json({ error: 'invalid requester' })
      ...
      if (!org) return res.status(404).json({ error: 'organization not found' })
        if (String(org.admin) !== String(requesterId)) return res.status(403).json({ error: 'forbidden' })
          // verify that the req and the admin are the sameee
          const body = req.body || {}
          const { doctorId } = body
    ...
    if (doctorId) {
      ...
      // emittt real-time notification to the doctor via Socket.IO if they're online
      try {
